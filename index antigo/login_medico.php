...
<head>
  ...
  <title>SisCOns - Login Médico</title>
...
include("Mensagens sweetAlerts Login/links_mensagens_sweetAlerts.php");
  ...
  if(isset($_POST["button"])){
   ...
   //$select = $mysqli->query("SELECT * FROM usuarios_n WHERE email='$email' AND senha='$senha'");
    $select = $mysqli->query("SELECT id, nome, email, nivel, senha, status FROM medico WHERE email='$email' AND senha='$senha'");
...
<?php
    ...
    if($row > 0){
      if($perm == 2 AND $status == 1){
      //session_start();
      $_SESSION["nivel"] = 2;
        echo '<script src="Mensagens sweetAlerts Login/mensagem_sweetAlert.js"></script>';
        sleep(2);
        echo '<script>window.location="paineis/painel_medico_consulta.php"</script>';
