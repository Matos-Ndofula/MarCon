...
if(isset($_GET["action"]) AND $_GET["action"] == "sair"){
    session_destroy();
    header("Location: ../novo_login_medico.php");
  }
?>
...
<body>
  ...
  <header id="header" class="header fixed-top d-flex align-items-center">
    ...
    <div class="d-flex align-items-center justify-content-between">
      <a href="novo_painel_medico.php" class="logo d-flex align-items-center">
  ...
  <!-- Sidebar -->
      ...
      <aside id="sidebar" class="sidebar">
        ...
        <ul id="forms-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav" id="consultas_abaixo">
         ...
         <a href="" data-bs-toggle="modal" data-bs-target="#Consultas_Desmarcadas" class="nav-link-custom_demarcar">
              <i class="bi bi-calendar-x"></i><span>Desmarcadas</span>
                      <?php 
...
?>
      ...
      <li class="nav-item">
        <a class="nav-link-custom  collapsed" href="" data-bs-toggle="modal" data-bs-target="#Agendamento_Medico">
          <i class="bi bi-person"></i>
          <span>Agendamento Medico</span>
        </a>
      </li><!-- End Profile Page Nav -->
        
        <a href="#" class="nav-link-custom logout-link" data-bs-toggle="modal" data-bs-target="#Pergunta_Sair"><i class="bi bi-box-arrow-right"></i> Sair</a>
  ...
  <main id="main" class="main">
    ...
    <section class="section dashboard">
       ...
       <a href="" data-bs-toggle="modal" data-bs-target="#Agendamento_Medico">
        <div class="action-card">
         ...
         <a href="" data-bs-toggle="modal" data-bs-target="#Consultas_Desmarcadas" id="consultas_desmarcadas">
            <div class="action-card">
              <div class="action-icon bg-add"><i class="bi bi-calendar-x"></i></div>
           
              <span class="fw-bold"> Consultas Desmarcadas</span>
                <span style="margin-left: 4px" class="badge bg-primary badge-number"><?php echo $totalConsultasDesmarcadas;?></span>
      </div>
</a>
   ...
   <!-- -->
    ...
    <div class="search-bar">
      <form class="search-form d-flex align-items-center" class="pesquisar" method="GET" action="novo_painel_medico.php">
    ...
    <?php
     ...
     $result_pesq_estado = "SELECT * FROM consultas_agendadas WHERE estado='Agendado' OR estado='Confirmada' OR estado='Atendido' OR estado<>'Desmarcada'";
    $resultado_pesquisa_estado = mysqli_query($mysqli, $result_pesq_estado);
   
    $result_pesq = "SELECT * FROM consultas_agendadas WHERE nome_utente LIKE '%$pesquisar%' ORDER BY data_horario DESC";
    ...
    while($rows_pesquisar = mysqli_fetch_array($resultado_pesquisa_estado)){
...
<a type="button" class="bi bi-check2-square" href="novo_painel_confirmar_agenda.php?id=<?php echo $id_agendado; ?>" id="botao_confirmar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Confirmar Consulta"></a>
...
$id_confirmada = encryptor('encrypt', $rows_pesquisar['id']);
  ...
  <a type="button" class="bi bi-clipboard2-check" href="novo_painel_atender_agenda.php?id=<?php echo $id_confirmada; ?>" id="botao_atender" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Atender Consulta"></a>
...
<!---INICIO MODAL PARA  CADATRAR DADOS PESSOAIS MÉDICO-->
    ...
    $result_pesq_desmarcada = "SELECT * FROM consultas_agendadas WHERE estado='Desmarcada' AND nome_medico='$nome'";
    $resultado_pesquisa__desmarcada = mysqli_query($mysqli, $result_pesq_desmarcada);
...
<!-- Tabela de Consultas -->
        ...
        while($rows_pesquisar_desmarcada = mysqli_fetch_array($resultado_pesquisa__desmarcada)){
              if ($rows_pesquisar_desmarcada['estado'] == 'Desmarcada'){ 
      ?>
                  <tr>
                    <td hidden><?php echo $rows_pesquisar_desmarcada['id']; ?></td>
                    
                    <td><?php echo $rows_pesquisar_desmarcada['nome_utente']; ?></td>
                    <?php ?>
                    <td><?php echo $rows_pesquisar_desmarcada['data_horario']; ?></td>
                    <td><?php echo $rows_pesquisar_desmarcada['horario']; ?></td>
                     <td><div  class="badge-status-desmarcada"><?php echo $rows_pesquisar_desmarcada['estado']; ?></div></td>  
                     <td>
                      <?php

            $id_elimina = encryptor('encrypt', $rows_pesquisar_desmarcada['id']);
      
                      ?>

      <a type="button" class="bi bi-trash" href="remover_consulta_medico.php?id=<?php echo $id_elimina; ?>" id="botao_remover" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Remover Consulta"></a> 
