...
require("config_encript_decrypt_url.php");
  ...
  $id = encryptor('decrypt', $id);
   ...
   $insert = $mysqli->query("INSERT INTO `consultas_canceladas`(`id_utente`,`nome_utente`,`especialidade`, `nome_medico`, `color`, `data_horario`, `estado`, `horario`) VALUES ('$id_utente','$nome_utente', '$especialidade', '$nome_medico', '$color', '$data_horario','$estado', '$horario')");
        }
...
';
  ...
  echo "<script>window.location='novo_painel_utente_desmarcar_consulta.php'</script>";
   ...
   $query_agendadas = "DELETE FROM consultas_agendadas WHERE id='".$id."'";
    if(mysqli_query($mysqli, $query_agendadas)){
  
} 
   $query_demarcadas = "DELETE FROM consultas_demarcadas WHERE id='".$id."'";
    if(mysqli_query($mysqli, $query_demarcadas)){
  ...
  /*
  $query = "UPDATE consultas_agendadas set estado = 'consulta_desmarcada' WHERE id='".$id."' ";

      
  if(mysqli_query($mysqli, $query)){
  echo '<script>alert("Consulta Desmarcada Actualizada");</script>
';
